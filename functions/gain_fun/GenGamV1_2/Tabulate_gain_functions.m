...
if gamma==2
    if nu <= 0
        ...
        [Gdft]=TabulateGenGam2dft(ksi_range,gam_range,nu);  %%tabulate the gain function for complex-DFT estimator for gamma=2
        [Gmag,Gmag2]=TabulateGainGamma2(ksi_range,gam_range,nu);%%tabulate the gain functions for DFT magnitude and magnitude squared estimatorestimator for gamma=2
    end
elseif gamma==1
    if nu <= 0.5
        ...
        [Gdft]=TabulateGenGam1dft(ksi_range,gam_range,nu,K); %%tabulate the gain function for complex-DFT estimator for gamma=1
        [Gmag,Gmag2]=TabulateGainGamma1(ksi_range,gam_range,nu,K);%%tabulate the gain functions for DFT magnitude and magnitude squared estimatorestimator for gamma=1
    end
