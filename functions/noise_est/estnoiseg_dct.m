function [x,zo]=estnoiseg_dct(yf,tz,pp,n_init)
...
if isempty(yf) && isstruct(tz)             % no real data
    ...
    if isstruct(tz)         % take parameters from a previous call
        ...
        qq.asnr= 15.6;         % active SNR in dB [15] (18)
        ...
        xt=n_init;                       % initialize just in case the first call has no data
    end

    psthr=qq.psthr;     % threshold for smoothed speech probability [0.99] (24)
    pnsaf=qq.pnsaf;     % noise probability safety value [0.01] (24)
    
    ax=0.9; % noise output smoothing factor = 0.8 (8) 0.94
    axc=1-ax;
    ap=0.9;% speech presence smoothing factor = 0.9 (23) 0.94
    apc=1-ap;
    
    % derived algorithm constants
    xih1=10^(qq.asnr/10); % speech-present SNR
    xih1r=1/(1+xih1)-1;
    pfac=(1/qq.pspri-1); % p(noise)/p(speech) (18)

    % loop for each frame
    for t=1:nr
        yft=yf(t,:);        % noisy speech power spectrum
        ph1y = (1+pfac*sqrt(1+xih1).*exp(xih1r.*yft./xt/2)).^(-1);% a-posteriori speech presence prob (18)
