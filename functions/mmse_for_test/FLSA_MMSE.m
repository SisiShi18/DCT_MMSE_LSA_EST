function [xfinal,sig_mod,n_mod]= FLSA_MMSE(varargin)
% Description:
...
default_opt = 'normal';
valid_opt = {'norm','lap', 'generalized-gamma','super-gaussian','gamma','normal','gauss','laplace','laplacian'};
check_opt = @(x) any(validatestring(x,valid_opt));
    ...
    case 'ideal'
        ...
        switch speech_prior
            case {'norm','normal','gaussian'}
                gain = FLSA_n(xi_frames,gamma_k);
            case {'super-gaussian'}
                for i = 1:length(L)
                    gain(i,:) = lookup_gain_in_table(LSA_sg,L{i,2}.',L{i,1}.',-40:1:40,-40:1:40,1);
                end 
            case {'generalized-gamma','gamma'}  
                gain =FLSA_g(xi_frames,gamma_k,zeta);
        end
...
%             gamma_k=min(noisy_pow(n,:)./noise_pow(n,:),40);
            ...
            if n==1  % initial condition for recursive computation
             ...
             switch speech_prior
                 case {'norm','normal','gaussian'}
                    gain =FLSA_n(xi,gamma_k);
                 case {'super-gaussian'}
                    gain = lookup_gain_in_table(FLSA_sg,gamma_k,xi,-40:1:40,-40:1:40,1);
                 case {'generalized-gamma','gamma'}
                    gain =FLSA_g(xi,gamma_k,zeta);
