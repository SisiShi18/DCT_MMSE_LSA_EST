function [xfinal,sig_mod,n_mod]= CLSA_MMSE_PE_ideal(varargin)
% Description:
...
d = r*c; % number of elements
%--------------------------------------------------------------------------
noise_mag = f_noise.abs;
clean_mag = f_clean.abs;
noise_pow_ideal = noise_mag.^2;
clean_pow = clean_mag.^2;

% xi_ideal = clean_pow./noise_pow_ideal;
...
%--------------------------------------------------------------------------
            ...
            gamma_k_ideal = sqrt(noisy_pow./noise_pow);
            xi_ideal = clean_mag./noise_mag;

            gamma_logic = gamma_k_ideal > 1;
            first = gamma_logic == 0;
            xi_logic = xi_ideal > 1;
%             xi_logic = sqrt(xi_ideal) > 1;
            ...
            acc_ideal = (sum(sum(temp))/d)*100; % accuracy
        else
...
%             gamma_k_ideal = sqrt(noisy_pow./noise_pow_ideal);
            gamma_k_ideal = noisy_pow./noise_pow_ideal;
            %             gamma_k_ideal= noisy_mag./noise_mag;

            xi_ideal = clean_mag./noise_mag;
            
%             gamma_k_ideal = sqrt(noisy_pow./noise_pow_ideal);
%             xi_ideal = sqrt(gamma_k_ideal.^2-1);
            
%             gamma_k_ideal = sqrt(xi_ideal.^2+1);
            gamma_logic = gamma_k_ideal > 1;
