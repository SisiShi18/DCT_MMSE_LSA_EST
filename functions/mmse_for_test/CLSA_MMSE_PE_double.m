function [xfinal,sig_mod,n_mod]= CLSA_MMSE_PE_double(varargin)
% Description:
...
%--------------------------------------------------------------------------
            case {'lap','laplace'}
                for i = 1:length(L)
                    gain(i,:) = lookup_gain_in_table(CLSA_l_gain_double,L{i,2}.',L{i,1}.',-40:1:40,-40:1:40,1);
                end
        end
...
%--------------------------------------------------------------------------                        
                        [xi,gamma_k] = est_xi_norm(noisy_pow(n,:),noise_pow(n,:),xi_min,alpha1,n,clean_est_frame);
%-------------------------------------------------------------------------- 
                        gain =lookup_gain_in_table(CLSA_l_gain_double,gamma_k,xi,...
                            -40:1:40,-40:1:40,1);
%--------------------------------------------------------------------------
                        gain = max(gain,gain_min);
